        ...
        <>
            ...
            >
                <span className="flex items-center gap-1.5">
                    <Icona className={`h-3.5 w-3.5 ${coloreIcona}`} />
                    {titolo}
                </span>
...
// modalità compatta le sostituiamo con classi fisse pensate per il mobile.
export default function DashboardContenuto({
    ...
    return (
        <div className={c('space-y-6 animate-fade-in', 'space-y-3 animate-fade-in')}>
            <div className={c(
                'relative overflow-hidden rounded-2xl bg-gradient-to-r from-green-600/20 via-emerald-600/20 to-slate-800/50 p-6 border border-slate-700/50',
                'relative overflow-hidden rounded-xl bg-gradient-to-r from-green-600/20 via-emerald-600/20 to-slate-800/50 p-3 border border-slate-700/50'
            )}>
                ...
                <h1 className={c('text-3xl font-bold text-white flex items-center gap-2', 'text-base font-bold text-white flex items-center gap-1.5')}>
                    <Landmark className={c('h-8 w-8 text-green-400', 'h-4 w-4 text-green-400')} />
            ...
            <div className={c('grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6', 'grid grid-cols-2 gap-2')}>
                {stats.map((stat) => (
                    <div key={stat.title} className={c(
                        'rounded-xl bg-slate-800/50 p-6 border border-slate-700/50 hover:border-green-500/50 transition hover:scale-105 cursor-pointer',
                        'rounded-lg bg-slate-800/50 p-2.5 border border-slate-700/50'
                    )}>
                        <div className="flex justify-between">
                            ...
                            <div className={c(
                                ...
                                <stat.icon className={c(`h-6 w-6 text-${stat.color}-400`, `h-3.5 w-3.5 text-${stat.color}-400`)} />
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <div className={c('grid grid-cols-1 lg:grid-cols-2 gap-6', 'grid grid-cols-1 gap-3')}>
                <div className={c('rounded-xl bg-slate-800/50 p-6 border border-slate-700/50', 'rounded-lg bg-slate-800/50 p-3 border border-slate-700/50')}>
            ...
            <div className={c('grid grid-cols-1 lg:grid-cols-3 gap-6', 'grid grid-cols-1 gap-3')}>
                ...
                <div className={c('rounded-xl bg-slate-800/50 p-6 border border-slate-700/50 col-span-2', 'rounded-lg bg-slate-800/50 p-3 border border-slate-700/50')}>
                    <h3 className={c('text-lg font-semibold text-white mb-4 flex items-center gap-2', 'text-xs font-semibold text-white mb-2 flex items-center gap-1.5')}>
                        <Clock className={c('h-5 w-5 text-orange-400', 'h-3.5 w-3.5 text-orange-400')} />
                        Prossime Scadenze
                    </h3>
            ...
            <div className={c('rounded-xl bg-slate-800/50 p-6 border border-slate-700/50', 'rounded-lg bg-slate-800/50 p-3 border border-slate-700/50')}>
                <h3 className={c('text-lg font-semibold text-white mb-4 flex items-center gap-2', 'text-xs font-semibold text-white mb-2 flex items-center gap-1.5')}>
                    <Eye className={c('h-5 w-5 text-cyan-400', 'h-3.5 w-3.5 text-cyan-400')} />
                    Bandi Consigliati
                </h3>
                {dashboard.gare_consigliate.length > 0 ? (
                    <div className={c('grid grid-cols-1 md:grid-cols-3 gap-4', 'grid grid-cols-1 gap-2')}>
                        {dashboard.gare_consigliate.map((gara) => (
                            <Link key={gara.id} href={`/ente/lista-bandi/${gara.id}`} className={c(
                                'rounded-xl bg-slate-900/50 p-4 border border-slate-700/50 block hover:border-cyan-500/50 transition',
                                'rounded-lg bg-slate-900/50 p-2.5 border border-slate-700/50 block'
                            )}>
                                <div className="text-right"><span className={c('px-2 py-1 rounded-full bg-green-500/20 text-green-400 text-xs', 'px-1.5 py-0.5 rounded-full bg-green-500/20 text-green-400 text-[10px]')}>Match {gara.match}%</span></div>
                                <h4 className={c('text-white font-semibold mt-2', 'text-white font-semibold mt-1 text-[11px]')}>{gara.titolo}</h4>
                                <p className={c('text-xs text-slate-400', 'text-[10px] text-slate-400')}>Budget: {formatEuro(gara.budget)}</p>
                                <span className={c('mt-3 text-sm text-green-400 inline-block', 'mt-1.5 text-[11px] text-green-400 inline-block')}>Dettagli →</span>
                            </Link>
                        ))}
                    </div>
