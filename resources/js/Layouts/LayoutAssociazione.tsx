...
import { LayoutDashboard, Bot, Search, Heart, Star, Settings, LogOut, Menu, X,
    Calendar, Users, Building2, Bell, HelpCircle, TrendingUp, BarChart3, Handshake } from 'lucide-react';

export default function LayoutAssociazione({ children }: { children: React.ReactNode }) {
    ...
    const navigation = [
        { name: 'Dashboard', href: '/dashboard/associazione', icon: LayoutDashboard },
        { name: 'Assistente AI', href: '/assistente', icon: Bot },
        { name: 'Bandi Finder', href: '/bandi', icon: Search },
        { name: 'Progetti Sociali', href: '/bandi/progetti', icon: Heart },
        { name: 'In Scadenza', href: '/bandi/scadenza', icon: Calendar },
        { name: 'Performance', href: '/associazione/performance', icon: BarChart3 },
        { name: 'Match Trovati', href: '/bandi/match', icon: TrendingUp },
        { name: 'Profilo Associazione', href: '/associazione/profilo', icon: Handshake },
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex relative">
            <div className="fixed inset-0 pointer-events-none">
                <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-pink-500/10 rounded-full blur-3xl animate-pulse-slow" />
                <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-rose-500/10 rounded-full blur-3xl animate-pulse-slow delay-1000" />
            ...
            <aside className={`fixed lg:relative z-50 w-64 bg-slate-900/95 backdrop-blur-sm border-r border-slate-700/50 h-full transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
                <div className="flex flex-col h-full">
                    ...
                    <div className="mx-4 mt-4 p-3 bg-gradient-to-r from-pink-500/20 to-rose-500/20 rounded-xl border border-pink-500/30">
                        <div className="flex items-center gap-2"><Heart className="h-4 w-4 text-pink-400" /><span className="text-xs text-pink-400 font-medium">Associazione / No-Profit</span></div>
                    ...
                    <div className="p-4 border-t border-slate-700/50 space-y-2">
                        <div className="flex items-center gap-3 px-4 py-2.5">
                            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center shadow-lg shadow-pink-500/30">
            ...
            <main className="flex-1 min-w-0 overflow-auto">
                <div className="sticky top-0 z-30 bg-slate-900/80 backdrop-blur-sm border-b border-slate-700/50 lg:hidden">
                    <div className="flex items-center p-3">
                        ...
                        <div className="flex-1 text-center"><span className="text-white font-bold">DeepAuto - Associazione</span></div>
                        <div className="w-6" />
                    </div>
                </div>
