...
export default function LayoutProfessionista({ children }: { children: React.ReactNode }) {
    ...
    const navigation = [
        { name: 'Dashboard', href: '/dashboard/professionista', icon: LayoutDashboard },
        { name: 'Assistente AI', href: '/assistente', icon: Bot },
        { name: 'Bandi Finder', href: '/bandi', icon: Search },
        { name: 'Opportunità', href: '/bandi/opportunita', icon: Briefcase },
        { name: 'In Scadenza', href: '/bandi/scadenza', icon: Calendar },
        { name: 'Performance', href: '/professionista/performance', icon: BarChart3 },
        { name: 'Match Trovati', href: '/bandi/match', icon: TrendingUp },
        { name: 'Profilo', href: '/professionista/profilo', icon: User },
        { name: 'Certificazioni', href: '/professionista/certificazioni', icon: Award },
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex relative">
            <div className="fixed inset-0 pointer-events-none">
                <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-amber-500/10 rounded-full blur-3xl animate-pulse-slow" />
                <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-orange-500/10 rounded-full blur-3xl animate-pulse-slow delay-1000" />
            ...
            <aside className={`fixed lg:relative z-50 w-64 bg-slate-900/95 backdrop-blur-sm border-r border-slate-700/50 h-full transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
                <div className="flex flex-col h-full">
                    ...
                    <div className="mx-4 mt-4 p-3 bg-gradient-to-r from-amber-500/20 to-orange-500/20 rounded-xl border border-amber-500/30">
                        <div className="flex items-center gap-2"><Briefcase className="h-4 w-4 text-amber-400" /><span className="text-xs text-amber-400 font-medium">Libero Professionista</span></div>
            ...
            <main className="flex-1 min-w-0 overflow-auto">
                <div className="sticky top-0 z-30 bg-slate-900/80 backdrop-blur-sm border-b border-slate-700/50 lg:hidden">
                    <div className="flex items-center p-3">
                        ...
                        <div className="flex-1 text-center"><span className="text-white font-bold">DeepAuto - Professionista</span></div>
                        <div className="w-6" />
                    </div>
                </div>
